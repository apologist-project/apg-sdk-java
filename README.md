...
```groovy
dependencies {
  implementation 'ai.apologist:apologist:1.0.4'
}
```
...
try{
    client.chat().createChatCompletion(...);
} catch (AgentClientApiException e){
...
```java
AgentClientHttpResponse response = client.chat().withRawResponse().createChatCompletion(...);
