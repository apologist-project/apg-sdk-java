...
sealed class JsonField<out T : Any> {
    ...
    fun asStringOrThrow(): String =
        asString().orElseThrow { ApologistInvalidDataException("Value is not a string") }
    ...
    internal fun getRequired(name: String): T =
        when (this) {
            is KnownValue -> value
            is JsonMissing -> throw ApologistInvalidDataException("`$name` is not set")
            is JsonNull -> throw ApologistInvalidDataException("`$name` is null")
            else -> throw ApologistInvalidDataException("`$name` is invalid, received $this")
        }
